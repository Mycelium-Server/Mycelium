...
MojangAPI::AuthResponse MojangAPI::requestAuth(ConnectionContext* ctx) {
    std::string url = toURL("https://sessionserver.mojang.com/session/minecraft/hasJoined?username="
            + ctx->playerData.name + "&serverId="
            + ((LoginPacketListener*) ctx->packetListener)->createHash());
